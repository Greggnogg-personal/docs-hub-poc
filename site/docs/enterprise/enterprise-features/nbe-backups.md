...
title: 'NetBox Enterprise: NetBox Enterprise Backups'
source: localdocs
lastUpdatedAt: 1765833193000
canonical: /docs/enterprise/enterprise-features/nbe-backups/
---
...
1. Navigate to the backup configuration by clicking the _^^Backup settings^^_ link in the **Disaster Recovery** section of the admin console.
   ![Backup settings](/enterprise-images/netbox-enterprise/netbox-enterprise-ec-backup-settings.png)
2. Input your S3 bucket, credentials, endpoint, and region.<br />
   ![Configure S3](/enterprise-images/netbox-enterprise/netbox-enterprise-ec-configure-s3.png)
3. Click **Update storage settings** -- it will spend a few moments validating that your settings work.<br />
   ![Update storage settings](/enterprise-images/netbox-enterprise/netbox-enterprise-ec-update-settings.png)
4. Perform a backup by clicking the **Backups** tab, and then clicking **Start backup**.<br />
   ![Start backup](/enterprise-images/netbox-enterprise/netbox-enterprise-ec-start-backup.png)
...
3. Enter the same S3 credentials you use for backups.<br />
   ![S3 Credentials](/enterprise-images/netbox-enterprise/netbox-enterprise-ec-restore-s3.png)
4. Next, it will take a few minutes to bring up the node.
   When it's complete, you will be prompted to continue the restore:<br />
   ![Restore from backup instance?](/enterprise-images/netbox-enterprise/netbox-enterprise-ec-restore-prompt.png)
...
7. Enter `continue` to finish bringing the NetBox Enterprise application up.<br />
   ![Continue restore](/enterprise-images/netbox-enterprise/netbox-enterprise-ec-restore-continue.png)
8. It will take a few more minutes to finish bringing the application up, and then you will see "Application restored!"<br />
   ![Application restored!](/enterprise-images/netbox-enterprise/netbox-enterprise-ec-restore-complete.png)<br />
...
See the [advanced tools documentation](./nbe-troubleshooting.md#command-line-access) for details on connecting to your NetBox Enterprise cluster.
...
1. Put NetBox Enterprise into "Restore Mode" by going to the _Config_ tab and checking the _Enable Restore Mode_ checkbox.
   ![Enable Restore Mode](/enterprise-images/netbox-enterprise/netbox-enterprise-restore-mode-enable.png)
2. Click the "Save config" button at the bottom of the form, and then when the admin console prompts you, click "go to updated version".<br />
   ![Go to updated version](/enterprise-images/netbox-enterprise/netbox-enterprise-restore-mode-updated-version.png)
3. Confirm that the _New version available_ at the top denotes it's a config change, and if so click the "Deploy" button.
   ![Deploy](/enterprise-images/netbox-enterprise/netbox-enterprise-restore-mode-deploy.png)
