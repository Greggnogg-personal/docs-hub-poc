...
canonical: /docs/integrations/tool-integrations/v1.2.6/servicenow/snow-user-guide/
---
...
**Buffer Files Priority Management** Files in the buffer will be named according to their destination data source (devices, sites, etc.), and are prefixed with a number (100, 200, 300). This number represents the order by which the data files will be moved to their destination data source. Specifically, all 100- files must be cleared out of the buffer before the 200- files will begin moving, and all of the latter must also be cleared before the 300- files are processed. This is done to honor dependencies, or references, between objects in a manner that respects a foundation-to-complex scheme, foundation objects (100-) being objects that have no references to other objects, dependent objects (200-) having references to foundation objects, and complex objects (300-) having references to dependent and/or foundation objects. The table below illustrates that concept.

|Level | Object type / file | References |
|-------|--------------------|------------|
|100 (foundation) | Customers |  |
|| Manufacturers |  |
|| Regions | Regions (see Region Hierarchy below) |
|200 (dependent) | Departments | Customers |
|| Device Types | Manufacturers |
|| Sites | Regions |
|300 (complex) | Devices | Customers |
|Departments |
|Device Types |
|Manufacturers |
|Sites |
 |
**Region Hierarchy**. Regions are unique in NetBox as they have a parent-child relationship with other regions, forming a hierarchy. To honor their dependencies to one another, they are themselves ordered within the 100- priority level, meaning that a region hierarchy will always be imported in a single 100-Regions file, in which the regions are ordered starting from the top-most region (ex. Americas) to the lowest (ex. San Diego West). Because import sets treat multiple entries sequentially from top to bottom, the hierarchy is preserved.
...
**Distinguishing Between Customer and Department Tenants**. The application supports two types of NetBox Tenants, distinguised by their belonging to Tenant Groups in NetBox which are specified in Application Parameters:

- NetBox Tenant Group for Customers
- NetBox Tenant Group for Departments
...
**Import and Export Parameters in ServiceNow**. All global, table-level controls appear in the Import and Export Application Parameters. These are made to allow (value = `true`) or deny (value = `false`) movement of object types between systems. For example, one could decide that the source of truth for Sites and Regions is in ServiceNow, therefore that these records should be exported to NetBox but never imported. This ensures a compliant flow of "truth" data in the selected direction.
...
The event queue is a record of all incoming webhooks sent from NetBox to ServiceNow. It is used to trigger validation and subsequent import of the data.
